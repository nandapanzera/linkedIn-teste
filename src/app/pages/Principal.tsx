...
export function Principal() {
  ...
  const toggleFavorite = (jobId: number) => {
    setFavorites((prev) => {
      const isFavorito = prev.includes(jobId);
      const updatedFavoritos = isFavorito
        ? prev.filter((favorito) => favorito !== jobId) // Remove do favoritos
        : [...prev, jobId]; // Adiciona aos favoritos
  ...
  const jobsToDisplay = showFavorites
    ? jobs.filter((job) => favorite.includes(job.job_id)) // Apenas favoritos
    : jobs;
